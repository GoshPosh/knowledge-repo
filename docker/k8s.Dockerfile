...
RUN cd ./knowledge-repo
COPY docker/pm_scripts/config.py config.py
RUN knowledge_repo --repo /knowledge-repo/repo init
ENTRYPOINT knowledge_repo --repo /knowledge-repo/repo deploy --config config.py --engine flask
